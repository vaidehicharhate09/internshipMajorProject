var express = require('express')    //create server to serve webpages
var ejs = require('ejs')
var bodyParser = require('body-parser');
var mysql = require('mysql');
var session = require('express-session');

var app = express();   //used to create local server
app.use(express.static('public'))    //to use public folder to deliver html
...
function calculateTotal(cart,req){
    total = 0;
    for(let i=0; i<cart.length; i++){
        //if we are offering discounted price
...
app.post('/place_order', function(req,res){
    ...
    con.connect((err)=>{
        if(err){
            ...
            var query = "INSERT INTO orders(id,cost,name,email,status,city,address,phone,date) VALUES ?";
            var values = 
            [[id,cost,name,email,status,city,address,phone,date]];
